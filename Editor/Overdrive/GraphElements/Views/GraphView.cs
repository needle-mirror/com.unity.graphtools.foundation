...
namespace UnityEditor.GraphToolsFoundation.Overdrive
{
    ...
    /// </summary>
    public class GraphView : VisualElement, IDragAndDropHandler, IDragSource
    {
        ...
        protected virtual void BuildContextualMenu(ContextualMenuPopulateEvent evt)
        {
            ...
            if (selection.Any())
            {
                ...
                {
                    CommandDispatcher.Dispatch(new AlignNodesCommand(this, GetSelection(), false));
                });
                ...
                evt.menu.AppendAction(itemName, _ =>
                {
                    CommandDispatcher.Dispatch(new AlignNodesCommand(this, GetSelection(), true));
                });
                ...
                var nodes = selection.OfType<INodeModel>().ToArray();
                if (nodes.Length > 0)
                {
                    ...
                    var ioConnectedNodes = connectedNodes.OfType<IInputOutputPortsNodeModel>()
                ...
                if (variableNodes.Length > 0)
                {
                    ...
                    evt.menu.AppendAction("Variable/Itemize",
                        _ => CommandDispatcher.Dispatch(new ItemizeNodeCommand(variableNodes.OfType<ISingleOutputPortNodeModel>().ToArray())),
                ...
                if (constants.Length > 0)
                {
                    ...
                    evt.menu.AppendAction("Constant/Itemize",
                        _ => CommandDispatcher.Dispatch(new ItemizeNodeCommand(constants.OfType<ISingleOutputPortNodeModel>().ToArray())),
                ...
                var edges = selection.OfType<IEdgeModel>().ToArray();
                if (edges.Length > 0)
                {
                    evt.menu.AppendSeparator();
                ...
                var stickyNotes = selection.OfType<IStickyNoteModel>().ToArray();
        ...
        {
            var selectedNodes = GetSelection().OfType<INodeModel>().ToArray();

            if (selectedNodes.Length == 0)
                return;

            var connectedNodes = selectedNodes.OfType<IInputOutputPortsNodeModel>().Where(x => x.InputsById.Values
            ...
            using (var selObservation = observer.ObserveState(CommandDispatcher.GraphToolState.SelectionState))
            {
                ...
                else if (rebuildType == UpdateType.Partial)
                {
                    ...
                    if (gvChangeSet != null && selChangeSet != null)
                    {
                        ...
                        combinedSet.AddRange(selChangeSet.ChangedModels.Except(gvChangeSet.DeletedModels));
