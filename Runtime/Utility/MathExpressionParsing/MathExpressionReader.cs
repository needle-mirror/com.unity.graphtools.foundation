...
{
    class MathExpressionReader
    {
        readonly string m_Input;
        int m_I;

        public MathExpressionReader(string input)
        {
            m_Input = input.Trim();
            m_I = 0;
        }
        ...
        public void ReadToken()
        {
            ...
            bool MatchOp(out Operator desc)
            {
                foreach (var pair in MathExpressionParser.Ops.Where(pair => m_Input.IndexOf(pair.Value.Str, m_I, StringComparison.Ordinal) == m_I))
                {
                    desc = pair.Value;
                    return true;
                }
