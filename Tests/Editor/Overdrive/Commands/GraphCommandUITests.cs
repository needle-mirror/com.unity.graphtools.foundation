...
namespace UnityEditor.GraphToolsFoundation.Overdrive.Tests.UI.Commands
{
    class GraphCommandUITests : BaseUIFixture
    {
        ...
        public IEnumerator DuplicateNodeAndEdgeCreatesEdgeToOriginalNode()
        {
            ...
            CommandDispatcher.Dispatch(new SelectElementsCommand(SelectElementsCommand.SelectionMode.Replace, nodeB, edge));
        ...
        public IEnumerator ReframeGraphViewWorks()
        {
            ...
            CommandDispatcher.Dispatch(new ReframeGraphViewCommand(newPos, newScale, new List<IGraphElementModel> { node }));
            yield return null;
        ...
        public IEnumerator FrameAllWorks()
        {
            ...
            CommandDispatcher.Dispatch(new ReframeGraphViewCommand(new Vector3(-500, -500, 0), new Vector3(.9f, .9f, .9f)));
            yield return null;
            ...
            foreach (var nodeModel in GraphModel.NodeModels)
            {
                ...
                Assert.IsTrue(GraphView.layout.Contains(nodeUI.layout.min),
                    $"Node {GraphModel.NodeModels.IndexOf(nodeModel)} min is outside graph view.");
                Assert.IsTrue(GraphView.layout.Contains(nodeUI.layout.max),
                    $"Node {GraphModel.NodeModels.IndexOf(nodeModel)} max is outside graph view.");
            }
        }
