...
namespace UnityEditor.GraphToolsFoundation.Overdrive.Tests.GraphElements
{
    class ShortcutSendsCommandTests : GraphViewTester
    {
        ...
        public IEnumerator ShortcutFrameAllSendsCorrectCommand()
        {
            ...
            void CommandHandler(GraphToolState state, ReframeGraphViewCommand command)
            {
            ...
            CommandDispatcher.RegisterCommandHandler<GraphToolState, ReframeGraphViewCommand>(CommandHandler);
        ...
        public IEnumerator ShortcutFramePreviousSendsCorrectCommand()
        {
            ...
            CommandDispatcher.Dispatch(new SelectElementsCommand(SelectElementsCommand.SelectionMode.Replace, elemList[k_NodeCount - 1]));
        ...
        public IEnumerator ShortcutFrameNextSendsCorrectCommand()
        {
            ...
            CommandDispatcher.Dispatch(new SelectElementsCommand(SelectElementsCommand.SelectionMode.Replace, elemList[0]));
        ...
        public IEnumerator ShortcutDeleteSendsCorrectCommand()
        {
            bool commandReceived = false;
            var elemList = graphView.GraphModel.NodeModels.Where(e => e.IsSelectable()).ToList();
            void CommandHandler(GraphToolState state, DeleteElementsCommand command)
            {
                commandReceived = true;
            ...
            CommandDispatcher.RegisterCommandHandler<GraphToolState, DeleteElementsCommand>(CommandHandler);
        ...
        public IEnumerator ShortcutConvertConstantToVariableSendsCorrectCommand()
        {
            ...
            void CommandHandler(GraphToolState state, ConvertConstantNodesAndVariableNodesCommand command)
            {
                commandReceived = true;
                Assert.AreEqual(1, command.ConstantModels.Count);
                Assert.AreEqual(0, command.VariableModels.Count);
                Assert.AreEqual(constantNode, command.ConstantModels[0]);
            }

            CommandDispatcher.RegisterCommandHandler<GraphToolState, ConvertConstantNodesAndVariableNodesCommand>(CommandHandler);

            CommandDispatcher.Dispatch(new SelectElementsCommand(SelectElementsCommand.SelectionMode.Replace, constantNode));
        ...
        {
            bool commandReceived = false;
            void CommandHandler(GraphToolState state, AlignNodesCommand command)
            {
                commandReceived = true;
                Assert.IsFalse(command.Follow);
            }

            CommandDispatcher.RegisterCommandHandler<GraphToolState, AlignNodesCommand>(CommandHandler);
        ...
        {
            bool commandReceived = false;
            void CommandHandler(GraphToolState state, CreateStickyNoteCommand command)
            {
                commandReceived = true;
            }

            CommandDispatcher.RegisterCommandHandler<GraphToolState, CreateStickyNoteCommand>(CommandHandler);
