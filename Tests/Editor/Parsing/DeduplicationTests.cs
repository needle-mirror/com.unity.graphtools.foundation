...
{
    class LoopDetectionTests : BaseDeduplicationLoopTestsFixture
    {
        ...
        public void LoopTest2()
        {
            ...
            CreateIfThenElseStacks(a, "b", "c", out var b, out var c);
            CreateIfThenElseStacks(b, "d", null, out var d, out var e);
            GraphModel.CreateEdge(b.InputPorts[0], c.OutputPorts[0]);
            GraphModel.CreateEdge(c.InputPorts[0], d.OutputPorts[0]);
        ...
        void AssertTranslationThrowsLoopDetectedException()
        {
            Assert.Throws<LoopDetectedException>(() => Stencil.CreateTranslator().TranslateAndCompile(GraphModel, AssemblyType.None, CompilationOptions.Default));
        }

        void AssertTranslationDoesNotThrowsLoopDetectedException()
        {
            Assert.DoesNotThrow(() => Stencil.CreateTranslator().TranslateAndCompile(GraphModel, AssemblyType.None, CompilationOptions.Default));
        }
    }
